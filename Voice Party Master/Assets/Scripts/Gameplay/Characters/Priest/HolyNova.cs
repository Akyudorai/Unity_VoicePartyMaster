...
public class HolyNova : MonoBehaviour
{
    ...
    void OnEnable() 
    {
        ...
        if (ps == null) {
            Debug.Log("HolyNova::Could not find ParticleSystem on " + gameObject.name);
        }
        ...
        for(int i = 0 ; i < targetTags.Count; i++)
        {
            ...
            foreach (GameObject colliderObject in allColliderGameObjects)
            {
                ...
                if (col == null) 
                {
                    Debug.Log("HolyNova::Could not find collider component on " + colliderObject.name);                
                }
        ...
        for (int i = 0; i < numInside; i++)
        {
            for (int j = 0; j < insideData.GetColliderCount(i); j++) 
            {
                ...
                if (!targetsHit.Contains(go)) 
                {
                    ...
                    // Heal all targets with the player tag
                    if (go.tag == "Player") 
                    {
                        // Restore Health to the Target based on Spell Power
                        go.GetComponent<PlayerController>().entity.DealDamage(-amount);                        
                    }
                }
            }
        }
